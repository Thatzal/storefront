/// status : 200
/// message : "Person or Place List"

class TrendingPersonModel {
  TrendingPersonModel({
      num? status, 
      String? message, 
      List<TrendingPersonData>? result,}){
    _status = status;
    _message = message;
    _result = result;
}

  TrendingPersonModel.fromJson(dynamic json) {
    _status = json['status'];
    _message = json['message'];
    if (json['result'] != null) {
      _result = [];
      json['result'].forEach((v) {
        _result?.add(TrendingPersonData.fromJson(v));
      });
    }
  }
  num? _status;
  String? _message;
  List<TrendingPersonData>? _result;
TrendingPersonModel copyWith({  num? status,
  String? message,
  List<TrendingPersonData>? result,
}) => TrendingPersonModel(  status: status ?? _status,
  message: message ?? _message,
  result: result ?? _result,
);
  num? get status => _status;
  String? get message => _message;
  List<TrendingPersonData>? get result => _result;
...
class TrendingPersonData {
  ...
  TrendingPersonData.fromJson(dynamic json) {
    _id = json['id'];
  ...
  String? _createdAt;
  String? _modified;
TrendingPersonData copyWith({  num? id,
  num? followers,
  num? following,
  String? displayname,
  String? phonenumber,
