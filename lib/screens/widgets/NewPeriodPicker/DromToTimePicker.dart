...
enum FromToTimePickerMode {
  time,
  date,
  dateAndTime,
}

class FromToTimePicker {
  final startText;
  final endText;
  final doneText;
  final cancelText;
  final bool use24hFormat;
  final FromToTimePickerMode mode;
  ...
  FromToTimePicker({
    ...
    this.mode = FromToTimePickerMode.dateAndTime,
    this.interval = 15,
  ...
  void showPicker(BuildContext context) {
   ...
   // ;
    ...
    if (initialEndTime == null) {
      initialEndTime = initialStartTime!.add(Duration(
          days: mode == FromToTimePickerMode.time ? 0 : 1,
          hours: mode == FromToTimePickerMode.time ? 2 : 0));
    }
   ...
   // ;
    ...
    switch (mode) {
      case FromToTimePickerMode.date:
        {
          pickerMode = CupertinoDatePickerMode.date;
        }
        break;

      case FromToTimePickerMode.time:
        {
                  ...
                  Container(
                    ...
                    child: TabBarView(
                      ...
                      children: widget._tabs.map((Tab tab) {
                          print(widget._initStart);
                          print(widget._initEnd);
                        return CupertinoTheme(
                          ...
                          child: CupertinoDatePicker(
                            ...
                            minimumDate: widget.type == "Today" ? tab.text == widget._tabs.first.text ? DateTime.now(): isChangeFromDate?TempFromDateTime:DateTime.now().add(Duration(minutes: 1)):
