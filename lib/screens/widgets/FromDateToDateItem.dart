...
typedef PickerConfirmCallbackItem = void Function(DateTime start, String end);

enum DateTimeRangePickerModeItemLavel {
  time,
  date,
  dateAndTime,
}

class DateTimeRangePickerItemLavel {
  final startText;
  final endText;
  final doneText;
  final cancelText;
  final bool use24hFormat;
  final DateTimeRangePickerModeItemLavel mode;
  final type;
  ...
  final PickerConfirmCallbackItem? onConfirm;

  final int interval;

  DateTimeRangePickerItemLavel({
    Key? key,
    this.startText = "Start",
    this.endText = "End",
    this.doneText = "Done",
    this.cancelText = "Cancel",
    this.mode = DateTimeRangePickerModeItemLavel.dateAndTime,
    this.interval = 15,
  ...
  void showPicker(BuildContext context) {
    ...
    if (initialEndTime == null) {
      initialEndTime = initialStartTime!.add(Duration(
          days: mode == DateTimeRangePickerModeItemLavel.time ? 0 : 1,
          hours: mode == DateTimeRangePickerModeItemLavel.time ? 2 : 0));
    }
    ...
    switch (mode) {
      case DateTimeRangePickerModeItemLavel.date:
        {
          pickerMode = CupertinoDatePickerMode.date;
        }
        break;

      case DateTimeRangePickerModeItemLavel.time:
        {
...
class PickerWidget extends StatefulWidget {
  ...
  final PickerConfirmCallbackItem? _onConfirm;
              ...
              child: TabBarView(
                ...
                children: widget._tabs.map((Tab tab) {
                  ...
                  return CupertinoDatePicker(
                    ...
                    // minimumDate: tab.text == widget._tabs.first.text ? DateTime.now(): TempFromDateTime,
                    // maximumDate: tab.text == "From"? widget._maximumFromTime:widget._maximumToTime,
