...
enum FromToDateRangePickerMode {
  time,
  date,
  dateAndTime,
}

class DatePickerFromTo {
  final startText;
  final endText;
  final doneText;
  final cancelText;
  final bool use24hFormat;
  final FromToDateRangePickerMode mode;
  ...
  DatePickerFromTo({
    ...
    this.mode = FromToDateRangePickerMode.dateAndTime,
    this.interval = 15,
  ...
  void showPicker(BuildContext context) {
    ...
    if (initialEndTime == null) {
      initialEndTime = initialStartTime!.add(Duration(
          days: mode == FromToDateRangePickerMode.time ? 0 : 1,
          hours: mode == FromToDateRangePickerMode.time ? 2 : 0));
    }
    ...
    switch (mode) {
      case FromToDateRangePickerMode.date:
        {
          pickerMode = CupertinoDatePickerMode.date;
        }
        break;

      case FromToDateRangePickerMode.time:
        {
                      ...
                      children: widget._tabs.map((Tab tab) {
                        ...
                        return CupertinoTheme(
                          ...
                          child: CupertinoDatePicker(
                            ...
                            minimumDate: tab.text == widget._tabs.first.text ? DateTime.now().subtract(Duration(hours: 1)): isChangeFromDate? TempFromDateTime: DateTime.now().subtract(Duration(hours: 1)),
                            maximumDate:   DateTime.now().add(Duration(days: 3225)),
